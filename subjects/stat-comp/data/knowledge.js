      ...
      },
      {
        ...
        content: [
          ...
          },
          {
            ...
            explain: '第1-3行：导入numpy用于数值计算和随机数生成，导入matplotlib用于直方图可视化，从scipy.stats导入norm获取标准正态的逆CDF函数(ppf)。<br>第5-7行：生成10000个U(0,1)均匀随机数作为逆变换的输入。<br>第9-11行(指数分布)：指数分布Exp(λ)的CDF为F(x)=1-e^{-λx}，求逆得F^{-1}(u)=-log(1-u)/λ。由于1-U与U同分布，等价于 -log(U)/λ。λ=2时理论均值为1/λ=0.5。<br>第13-14行(正态分布)：scipy的norm.ppf()就是标准正态的逆CDF(分位数函数)。直接将U代入ppf即可得到服从N(0,1)的随机数——无需自己求逆公式。<br>第16-24行：并排绘制两种分布的直方图与理论PDF曲线，直观验证逆变换法的正确性。<br>第26-28行：打印样本统计量，对比理论值验证采样精度。',
      ...
      },
      {
        ...
        content: [
          ...
          },
          {
            ...
            explain: '第1-8行：用<code>np.random.choice</code>按比例分配组分标签，再用<code>multivariate_normal</code>生成各组分样本。<br>第10-15行：定义多元高斯PDF函数<code>gaussian_pdf</code>——计算(X-μ)^T Σ^{-1} (X-μ)的二次型，除以归一化常数√((2π)^d|Σ|)，这是E步中计算组分似然的核心。<br>第17-20行：初始化参数——π均匀、μ从数据中随机选K个点、协方差初始化为1.5I。<br>第22-38行(EM主循环)：<strong>E步</strong>——对每个样本和组分计算π_k·N(x_i|μ_k,Σ_k)，归一化得责任度γ_{ik}(软标签)；计算对数似然logL作为收敛监控。<strong>M步</strong>——N_k=∑γ_{ik}为组分k的有效样本数，π_k=N_k/n，μ_k用γ_{ik}加权平均，Σ_k用γ_{ik}加权协方差。迭代至logL变化<1e-6停止。加1e-300防止log(0)的数值问题。',
